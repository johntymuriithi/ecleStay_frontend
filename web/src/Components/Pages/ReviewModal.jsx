...
const ReviewModal = ({ show, onClose, order }) => {
  // const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [rating, setRating] = useState('');
  const [cleanliness, setCleanliness] = useState('');
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${show ? 'block' : 'hidden'}`}>
      <div className="bg-blue-100 rounded-lg p-6 w-full max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {error && <p className="col-span-2 text-green-500 text-center mb-1">{error}</p>}
          {successMessage && <p className="success col-span-2 text-red-500 text-center mb-1">{successMessage}</p>}
          <div className="col-span-2 mt-6 flex justify-end space-x-4">
            <button
              type="button"
              onClick={onClose}
              className="py-2 px-4 bg-gray-600  text-white rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-700"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="py-2 px-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:bg-blue-700"
            >
              Submit Review
            </button>
          </div>
